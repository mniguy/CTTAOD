    ...
    with EventStorage() as storage:
        for idx, inputs in enumerate(data_loader):
            ...
            if cur_used or (not cur_used and pause_iter % cfg.TEST.ADAPTATION.SKIP_PERIOD == 0) or ('period' in cfg.TEST.ADAPTATION.SKIP_REDUNDANT and idx % cfg.TEST.ADAPTATION.SKIP_PERIOD == 0):
                ...
                # weight regularization (L2 anchor to source weights)
                ...
                if proto_replay and hasattr(model, 'prototype_buffer') and model.prototype_buffer:
                    ...
                    for k in model.t_stats.get("fg", {}):
                        ...
                        for buf_mean in model.prototype_buffer.get(k, []):
                            replay_loss = replay_loss + torch.mean((cur_mean - buf_mean.to(cur_mean.device)) ** 2)
                            n_replay += 1
                    if n_replay > 0:
                        losses["proto_replay"] = replay_loss / n_replay
                ...
                if total_loss > 0 and cur_used:
                    ...
                    if stoch_restore:
                        ...
                        for p, s in zip(param_list, init_weights):
                            ...
                            p.data[mask] = s.to(p.device)[mask]
                else:
                    pause_iter = 1
                optimizer.zero_grad()
                # EMA update
