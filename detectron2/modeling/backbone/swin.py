...
from detectron2.layers import ShapeSpec, LayerNorm
...
class MultiAdapter(nn.Module):
    def __init__(self,
                 ...
                 r=32):
        super().__init__()
        self.adapters = nn.ModuleList([Adapter(config=config, d_model=d_model, bottleneck=bottleneck) for _ in range(config.NUM_EXPERTS)])
        self.num_experts = config.NUM_EXPERTS
        self.top_k = config.TOP_K
        self.tau = config.ROUTING_TEMPERATURE

        self.proj = nn.Linear(d_model, r)
        self.codes = nn.Parameter(torch.zeros(self.num_experts, r))
        nn.init.kaiming_uniform_(self.proj.weight, a=math.sqrt(5))
        nn.init.kaiming_uniform_(self.codes, a=math.sqrt(5))

    def routing(self, x):
        logits = F.cosine_similarity(self.codes, self.proj(x.mean(dim=[0,1])).unsqueeze(0))
        probs = nn.Softmax(dim=1)(logits.unsqueeze(0) / self.tau)
        return probs.max(dim=1)

    def forward(self, x):
        weight, idx = self.routing(x)
        return weight * self.adapters[idx](x)


class MOEAdapter(MOELayer):
    def __init__(self,
        gate_type,
        model_dim: int,
        experts=None,
        ...
        use_2dh=False,
        **kwargs):
        super().__init__(gate_type, model_dim, experts, scan_expert_func, result_func, group, seeds,
                         a2a_ffn_overlap_degree, is_postscore, batch_prioritized_routing, normalize_gate,
                         is_gshard_loss, parallel_type, use_2dh)
        with torch.no_grad():
            nn.init.kaiming_uniform_(self.experts.batched_fc1_w, a=math.sqrt(5))
            nn.init.zeros_(self.experts.batched_fc2_w)
            nn.init.zeros_(self.experts.batched_fc1_bias)
            nn.init.zeros_(self.experts.batched_fc2_bias)
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        ...
        for name, param in self.experts.named_parameters():
            ...
            if buff_name in state_dict and state_dict[buff_name].numel() == param.numel():
                state_dict[buff_name] = state_dict[buff_name].view(param.shape)
        return super(MOELayer, self)._load_from_state_dict(state_dict, prefix, *args, **kwargs)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_layer(dim, out_batch_norm=out_batch_norm, out_batch_resolution=out_batch_resolution)
        ...
        self.norm2 = norm_layer(dim, out_batch_norm=out_batch_norm, out_batch_resolution=out_batch_resolution)
        ...
        if adapting_config.TYPE is not None:
            if adapting_config.TYPE == "MoE":
                self.adapter = MOEAdapter(
                    gate_type={'type': adapting_config.ROUTER, 'k': 1, 'fp32_gate': True, 'gate_noise': 1.0, 'capacity_factor': 1.5},       # router: 'cosine_top'
                    experts={'type': 'ffn', 'count_per_node': adapting_config.NUM_EXPERTS,
                             'hidden_size_per_expert': dim//adapting_config.HIDDEN_RATIO,
            ...
            elif adapting_config.TYPE == "MultiAdapter":
                self.adapter = MultiAdapter(adapting_config, d_model=dim, bottleneck=dim//adapting_config.HIDDEN_RATIO)
            else:
                self.adapter = Adapter(adapting_config, d_model=dim, bottleneck=dim//adapting_config.HIDDEN_RATIO)
        else:
            self.adapter = None
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.adapter is not None:
            ...
            # x = self.norm2(x) + self.drop_path(self.mlp(self.norm2(x))) + adapt_x
            x = x + self.drop_path(self.mlp(self.norm2(x))) + adapt_x
        else:
            # x = self.norm2(x)
            if hasattr(self.norm2, 'out_batch_norm') and self.norm2.out_batch_norm:
                self.norm2.update(self.norm2(x).view(B, H, W, C).clone())
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, out_batch_resolution=32):
        super().__init__()
        self.dim = dim
        ...
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    adapting_config=adapting_config,
                    out_batch_norm=out_batch_norm,
                    out_batch_resolution=out_batch_resolution
                )
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                adapting_config=adapting_config,
                out_batch_norm=out_batch_norm,
                out_batch_resolution=2 ** (8 - i_layer))
...
def build_swint_backbone(cfg, input_shape):
    ...
    return SwinTransformer(
        ...
        adapting_config=cfg.TEST.ADAPTER,
